---
name: copilot
description: Delegate a coding task to a GitHub Copilot CLI session managed via tmux. Invoked as /copilot. User drives all progress checks and interventions — no auto-polling.
---

# /copilot — delegate to GitHub Copilot CLI

Spawns Copilot CLI in a detached tmux session so Claude can offload work to it. The user stays in the loop: they ask Claude when to check progress, what guidance to send, and when to stop.
...
```
~/.claude/skills/copilot/bin/copilot-agent [--session <name>] <verb> [args]
```

Verbs:

- `start [task...]`  — create session (default `cc-copilot`), launch `copilot --allow-all`, type the task and press Enter. Returns immediately (non-blocking).
...
- `wait [stable] [timeout]` — **block until copilot goes idle, then exit.** Run via a backgrounded Bash so the harness notifies Claude on completion (see "Auto-notify" below). `stable` = secs the working indicator must stay gone before declaring idle (default 6); `timeout` = overall cap in secs (default 1800).
- `stop`             — kill-session. Destroys the tmux session.
- `list`             — list active `cc-copilot*` sessions.
...
- Default session name: `cc-copilot`. For concurrent tasks, pass `--session <name>` before the verb.
...
```
Bash(
  command: "~/.claude/skills/copilot/bin/copilot-agent wait",
  run_in_background: true,
  description: "wait for copilot to go idle"
)
```
